...
class Graph:  # tube network
    ...
    def get_edge_between(self, vertex1, vertex2):
        if vertex1.station_id not in vertex2.neighbours:
            ...
            for edge_id in vertex1.edges:
                ...
                if vertex2.station_id in [edge.vertex2, edge.vertex1] and vertex1.station_id in [edge.vertex2,
                                                                                                 edge.vertex1]:
                    return edge
    ...
    def find_route(self, start, end):
        ...
        start_hub_id = int(stations_codelist.loc[start]['hub_id'])  # 198
        ...
        while not end.visited:
            ...
            for neighbour_id in current.neighbours:
                ...
                if edge_to_neighbour.time == time_to_change_lines and (
                        edge_to_neighbour.vertex1 == start.station_id or edge_to_neighbour.vertex2 == end.station_id):
        ...
        def plot_route(route_to_end=routes[end.station_id]):
            ...
            get_line_colour = lambda line_id: '#999999' if line_id is None else str('#' + lines.l[line_id]['colour'])
            get_legend = lambda line_id: None if line_id is None else str(lines.l[line_id]['name'])
            ax = plt.subplot()
            for i in range(1,
                           len(route_to_end)):  # loop through a list of dictionaries, one for each station on the
                # selected route
                ...
                plt.plot([edge.coordinates['long_from'], edge.coordinates['long_to']],
                         [edge.coordinates['lat_from'], edge.coordinates['lat_to']], marker='o', linestyle='--',
                         color=line_colour, label=mylegend)
                ax.annotate(vertex1.name, xy=(edge.coordinates['long_from'], edge.coordinates['lat_from']),
                            xytext=(edge.coordinates['long_from'] + 0.002, edge.coordinates['lat_from'] - 0.002))
                ax.annotate(vertex2.name, xy=(edge.coordinates['long_to'], edge.coordinates['lat_to']),
                            xytext=(edge.coordinates['long_to'] + 0.002, edge.coordinates['lat_to'] - 0.002))
...
stations_codelist = stations_df[['hub_id', 'name']].set_index('name', drop=True, append=False, inplace=False,
                                                              verify_integrity=True)  # print(stations_codelist.loc[
# 'Bank']['id'])
...
for i in range(len(bigtab)):
    ...
    all_vertices[tuple([row['station1'], row['line']])] = Vertex(row['name'], row['station1'],
                                                                 tuple([row['station1'], row['line']]), row['latitude'],
                                                                 row['longitude'])
    all_vertices[tuple([row['station2'], row['line']])] = Vertex(stations_df.loc[row['station2']]['name'],
                                                                 row['station2'], tuple([row['station2'], row['line']]),
                                                                 stations_df.loc[row['station2']]['latitude'],
                                                                 stations_df.loc[row['station2']]['longitude'])
    all_edges[i] = Edge(row['edge_id'], tuple([row['station1'], row['line']]), tuple([row['station2'], row['line']]),
                        row['time'], row['line'])
...
print(tube.find_route('South Wimbledon', 'Moor Park'))
