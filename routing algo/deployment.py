...
class Graph:  # tube network
    ...
    def findRoute(self, start, end):
        ...
        try:
            ...
            while not end.visited:
                ...
                for neighbour_id in current.neighbours:
                    ...
                    if neighbour.station_id in unvisited:
                        if unvisited[neighbour.station_id] > (
                                unvisited[current.station_id] + connection_to_neighbour.time):
                            unvisited[neighbour.station_id] = unvisited[
            ...
            messagebox.showinfo("showinfo",f"Journey from {start.name} to {end.name} will take approximately {visited[-1]} minutes.")
...
def getInputs():
    ...
    Graph().findRoute(source_value.get(), destination_value.get())
