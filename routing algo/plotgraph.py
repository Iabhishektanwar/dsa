...
class Graph:  # tube network
    ...
    # type Graph, type Connection, tuple representing station.station_id
    ...
    def findRoute(self, start, end):
        ...
        unvisited = dict([(station_hub_id, 1000)
                         for (station_hub_id, time) in self.all_stations.items()])
        unvisited[start.station_id] = 0  # (198, 11)
        visited = []
        current = start
        routes = {current.station_id: [
            {'name': current.name, 'station_id': current.station_id}]}
        ...
        while not end.visited:
            ...
            for neighbour_id in current.neighbours:
                ...
                connection_to_neighbour = self.getConnectionBetween(
                    current, neighbour)  # []
                ...
                if neighbour.station_id in unvisited:
                    if unvisited[neighbour.station_id] > (unvisited[current.station_id] + connection_to_neighbour.time):
                        unvisited[neighbour.station_id] = unvisited[current.station_id] + \
        ...
        def plot_route(route):
            ...
            def getLineColour(line_id): return '#999999' if line_id is None else str(
                '#' + lines[line_id]['colour'])
            def getLineName(line_id): return None if line_id is None else str(
                lines[line_id]['name'])
            ax = plt.subplot()
            ...
            for r in range(1,
                ...
                plt.plot([connection.coordinates['long_from'], connection.coordinates['long_to']],
                         [connection.coordinates['lat_from'],
                             connection.coordinates['lat_to']], marker='o',
                         linestyle='--',
            ...
            ax.set_xlim(
                (coordinates_limits['min_long'] - 0.02), (coordinates_limits['max_long'] + 0.02))
            ax.set_ylim(
                (coordinates_limits['min_lat'] - 0.02), (coordinates_limits['max_lat'] + 0.02))
...
for i in range(len(master_data)):
    ...
    all_connections[i] = Connection(master_data[i]['connection_id'],
                                    tuple([master_data[i]['station1'],
                                          master_data[i]['line']]),
                                    tuple(
                                        [master_data[i]['station2'], master_data[i]['line']]), master_data[i]['time'],
                                    master_data[i]['line'])

# connect class Station with class Connection by filling station.neighbours and station.connections:
for i in range(len(master_data)):
    station_to_update = all_stations[tuple(
        [master_data[i]['station1'], master_data[i]['line']])]
    station_to_update2 = all_stations[tuple(
        [master_data[i]['station2'], master_data[i]['line']])]
    station_to_update.connections.append(i)
    station_to_update2.connections.append(i)
    station_to_update.neighbours.append(
        tuple([master_data[i]['station2'], master_data[i]['line']]))
    station_to_update2.neighbours.append(
        ...
        for s in hub:
            if station != s:
                ...
                all_connections[i] = Connection(
                    i, station, s, time_to_change_lines, None)
                i += 1

# call the findRoute function
print(Graph().findRoute('Alperton', 'Angel'))
